...
opportunityMain.OnLoadForm = function () {  
    try {
        ...
        function jQueryIsReady() {
            ...
            /* Market Sector */
             if(Xrm.Page.getAttribute("new_solutions").getSelectedOption().text == "Academy Solutions" ||Xrm.Page.getAttribute("new_solutions").getSelectedOption().text == "Ship Solutions" ||Xrm.Page.getAttribute("new_solutions").getSelectedOption().text =="Ship Traffic Control Solutions" ||Xrm.Page.getAttribute("new_solutions").getSelectedOption().text =="Fleet Operation Solutions") {
                ...
                parent.$.each(markets, function (key, value) {
                    parent.$('#new_marketsector_i').append($("<option></option>").attr("value", value).attr("title", key).text(key));
                });
                ...
                parent.$.each(parent.$('#new_marketsector_i option'), function () {
                    var found = false;
                ...
                parent.$("#new_marketsector_i").parents("tr:first").show();
            }
            else {
                parent.$('#new_marketsector_i option').remove();
...
function EstimatedOrderDateOnSave() {
    ...
    if (!Xrm.Page.getAttribute("new_actualorderdate").getIsDirty())
        return;
    ...
    if (now > estOrderDate) {
     //Do not Save when Est.Order Date is in the past
        Xrm.Page.ui.setFormNotification("Please provide for Est. Order Date a future date", "ERROR","1");
...
function new_solutions_onchange() {
    try {
        debugger;
        // only for solutions which market sectors
        if (jQuery.inArray(parent.$('#new_solutions_i option:selected').text(), ["Ship Solutions", "Ship Traffic Control Solutions", "Fleet Operation Solutions", "Academy Solutions"]) > -1) {
            ...
            //set the defualt value to Unasssigned for all selected optionset
            Xrm.Page.getAttribute("new_marketsector").setValue(400);

            var arr = EmptySolutions;
            if (parent.$('#new_solutions_i option:selected').text() == 'Ship Solutions') arr = ShipSolutions;
            if (parent.$('#new_solutions_i option:selected').text() == 'Ship Traffic Control Solutions') arr = ShipTrafficControlSolutions;
            if (parent.$('#new_solutions_i option:selected').text() == 'Fleet Operation Solutions') arr = FleetOperationSolutions;
            if (parent.$('#new_solutions_i option:selected').text() == 'Academy Solutions') arr = AcademySolutions;
        ...
        /* Show ECDIS QTY*/
        parent.$('#new_solutions_i option:selected').text() == "Ship Solutions" ? parent.$('#new_ecdiswsqty').parents("tr:first").show() : parent.$('#new_ecdiswsqty').parents("tr:first").hide();
    }
    catch (e) {
        console.log(e.message);
    }
...
//        /* Market Sector */
//        // only for solutions which market sectors
//        if (Xrm.Page.getAttribute("new_solutions").getSelectedOption().text == "Ship Solutions" || "Ship Traffic Control Solutions" || "Fleet Operation Solutions" || "Academy Solutions") {
//            var markets = { "Unassigned": 400, "Coast Guard/Police": 10, "Coastal Surveillance System": 20, "Container ship": 30, "Cruise/Passenger Ship": 40, "Dry Cargo other ": 50, "Fishing": 60, "Fishing Vessel": 70, "Inland": 80, "Mega Yacht (SOLAS)": 90, "Naval": 100, "Navy": 110, "Offshore": 120, "Offshore Energy Project": 130, "Offshore Supply (OSV)": 140, "Other None SOLAS": 150, "Other STC project/contract": 160, "Pilots": 170, "R&D": 180, "River Information System": 190, "Ro-Ro/Ro-PAX/Car": 200, "Safety": 210, "SAR": 220, "SAR/GMDSS System": 230, "Ship manager": 240, "Shipowner": 250, "STCW, Others": 260, "Tanker (LNG/LPG)": 270, "Tanker (Oil)": 280, "Tanker (Product)": 290, "Tug/Workboat": 300, "Vessel Traffic System": 310, "WebSim": 320 };
//            var ShipSolutions = new Array(10, 30, 40, 50, 70, 90, 100, 140, 150, 200, 220, 270, 280, 290, 300, 400);
//            var ShipTrafficControlSolutions = new Array(20, 130, 160, 190, 230, 310, 400);
//            var FleetOperationSolutions = new Array(240, 250, 400);
