...
opportunityMain.OnLoadForm = function () {
    ...
    try {
        ...
        function jQueryIsReady() {
            ...
            if (jQuery.inArray(parent.$('#new_solutions option:selected').text(), ["Ship Solutions", "Ship Traffic Control Solutions", "Fleet Operation Solutions", "Academy Solutions"]) > -1) {
                ...
                parent.$('#new_marketsector option').each(function () {
                    var found = false;
    ...
    try {
        ...
        if (jQuery.inArray($('#new_solutions option:selected').text(), ["Ship Solutions", "Ship Traffic Control Solutions", "Fleet Operation Solutions", "Academy Solutions"]) > -1) {
            ...
            $.each(markets, function (key, value) {
                $('#new_marketsector').append($("<option></option>").attr("value", value).attr("title", key).text(key));
            });
...
function CFSTLProjectID_onchange() {
    try {
        alert('Check Please');
        if (crmForm.all.CFSTLProjectID = null) {
...
function new_tender_onchange() {
    try {
        if (Xrm.Page.getAttribute("new_tender").getSelectedOption == true)
            {
            Xrm.Page.ui.tabs.get("tab1Tab").setVisible("true");
        } else {
            Xrm.Page.ui.tabs.get("tab1Tab").setVisible("false");
        }
    }
    catch (e) {
        console.log(e.message);
...
function new_projecttype_onchange() {
    try {
        ...
        if (Xrm.Page.getAttribute("new_projecttype").getSelectedOption == InvestmentTextValue) {
            Xrm.Page.ui.tabs.get("tab5Tab").setVisible("true");
        }
        else {
            Xrm.Page.ui.tabs.get("tab5Tab").setVisible("false");
        }
        // END OF INVESTMENT TAB
    }
    catch (e) {
    ...
    try {
        ...
        if ((Xrm.Page.ui.getFormType() == CRM_FORM_TYPE_CREATE)) {
            if (Xrm.Page.getAttribute("new_department").getSelectedOption == 'Navigation') {
                ...
                // show Navigational section
                Xrm.Page.ui.tabs.get("tab0").sections.get("Navigational Project").setVisible(true);
            }
            else {
                // hide Navigational section
                Xrm.Page.ui.tabs.get("tab0").sections.get("Navigational Project").setVisible(false);
            }
        }
    }
    catch (e) {
